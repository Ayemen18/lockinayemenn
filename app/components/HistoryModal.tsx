      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="bg-neutral-900 border border-neutral-800 rounded-2xl max-w-md w-full p-6 relative overflow-hidden z-10 shadow-2xl flex flex-col max-h-[85vh] font-sans"
          >
            
            {/* Header info */}
            <div className="flex items-start justify-between border-b border-neutral-800/80 pb-4 mb-4">
              <div>
                <span className="text-[10px] text-neutral-500 font-mono uppercase tracking-widest block mb-0.5">Historical Log</span>
                <h3 className="text-sm font-bold text-white tracking-tight">{formattedDate}</h3>
              </div>
              <button
                onClick={onClose}
                className="w-7 h-7 rounded-lg hover:bg-neutral-800 border border-transparent hover:border-neutral-700/50 flex items-center justify-center text-neutral-400 hover:text-white transition-all outline-none"
              >
                ✕
              </button>
            </div>
              ...
              {!log ? (
                ...
                <>
                  ...
                  <div className="bg-neutral-950 border border-neutral-850 p-4 rounded-xl flex items-center justify-between">
                    <div>
                      <p className="text-[10px] text-neutral-600 font-mono uppercase tracking-widest">Completion score</p>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-2xl font-bold tracking-tight ${
                        log.score >= 80 ? 'text-green-400' : log.score >= 60 ? 'text-emerald-500/80' : 'text-neutral-400'
                      }`}>
                        {log.score}%
                      </span>
                    </div>
                  ...
                  <div className="space-y-2.5">
                    <p className="text-[10px] text-neutral-600 font-mono uppercase tracking-widest">Habit checklist</p>
                    
                    <div className="border border-neutral-800/60 rounded-xl overflow-hidden divide-y divide-neutral-800/40">
                      {habits.map(habit => {
                        const isCompleted = log.completed_habit_ids?.includes(habit.id)
                        return (
                          <div
                            key={habit.id}
                            className={`flex items-center gap-3 px-4 py-3 bg-neutral-900/40 text-xs ${
                              isCompleted ? 'bg-neutral-900/10' : ''
                            }`}
                          >
                            {/* Checkmark icon indicator */}
                            <div className={`w-4.5 h-4.5 rounded-full flex items-center justify-center flex-shrink-0 border ${
                              isCompleted ? 'border-green-500/40 bg-green-950/40 text-green-400' : 'border-neutral-800 text-neutral-700'
                            }`}>
                              {isCompleted ? '✓' : '○'}
                            </div>
                            ...
                            <span className={`flex-1 font-medium ${
                              isCompleted ? 'text-neutral-500 line-through' : 'text-neutral-350'
                            }`}>
                              {habit.name}
                            </span>
                  ...
                  <div className="grid grid-cols-2 gap-3.5">
                    <div className="bg-neutral-950 border border-neutral-850/80 p-3.5 rounded-xl">
                      <p className="text-[10px] text-neutral-600 font-mono uppercase tracking-widest mb-1.5">Study Focus</p>
                      <p className="text-sm font-bold text-neutral-200">
                        {log.study_hours !== undefined && log.study_hours !== null 
                          ? `${log.study_hours} hrs` 
                          : <span className="text-xs text-neutral-600 font-mono">Not logged</span>
                        }
                      </p>
                    </div>

                    <div className="bg-neutral-950 border border-neutral-850/80 p-3.5 rounded-xl">
                      <p className="text-[10px] text-neutral-600 font-mono uppercase tracking-widest mb-1.5">Sleep logged</p>
                      <p className="text-sm font-bold text-neutral-200">
                        {log.sleep_time
                          ? `${log.sleep_time} hrs` 
                          : <span className="text-xs text-neutral-600 font-mono">Not logged</span>
                  ...
                  {/* Journal Reflection */}
                  <div className="bg-neutral-950 border border-neutral-850/80 p-4 rounded-xl">
                    <p className="text-[10px] text-neutral-600 font-mono uppercase tracking-widest mb-2">Daily reflection</p>
                    {log.reflection ? (
                      <p className="text-xs text-neutral-300 leading-relaxed italic">
                        "{log.reflection}"
                      </p>
                    ) : (
                      <p className="text-xs text-neutral-600 italic font-mono">No reflection entry recorded.</p>
                    )}
                  </div>
                </>
              )}
            ...
            <div className="mt-5 pt-3.5 border-t border-neutral-800/80 flex justify-end">
              <button
                onClick={onClose}
                className="bg-neutral-800 hover:bg-neutral-700 text-xs font-semibold px-4 py-2 rounded-xl text-white transition-colors"
              >
                Done
              </button>
            </div>
